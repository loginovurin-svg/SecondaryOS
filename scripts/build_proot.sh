...
# КЛЮЧЕВОЙ МОМЕНТ: находим linker64 (ld-android.so)
ld_src=$(find $SYSROOT/usr/lib/aarch64-linux-android -name "ld-android.so" 2>/dev/null | head -1)
if [ -z "$ld_src" ]; then
    echo "ВНИМАНИЕ: ld-android.so не найден, пробуем статическую линковку..."
    USE_STATIC=1
else
...
if [ "$USE_STATIC" = "1" ]; then
    # Статическая линковка — не нужен linker64
    ...
    # Динамическая линковка — нужен linker64
...
if echo "$QEMU_OUT" | grep -q "qemu works"; then
    ...
    echo "  ✗ qemu не работает, переключаемся на cross-answers.txt"
    QEMU_CMD=""
fi
...
if [ -n "$QEMU_CMD" ]; then
    # Используем qemu для cross-execute
    ...
    # Фолбэк: cross-answers.txt
    echo "Режим: cross-answers.txt (qemu недоступен)"
