...
# Зависимость: talloc (управление памятью из Samba)
# Результат: app/src/main/jniLibs/arm64-v8a/libproot.so
# ============================================================

# Путь к NDK (устанавливается через GitHub Actions setup-android)
if [ -z "$ANDROID_NDK_HOME" ]; then
    # Фолбэк для стандартного пути GitHub Actions
    export ANDROID_NDK_HOME=$(ls -d /usr/local/lib/android/sdk/ndk/* 2>/dev/null | head -1)
    if [ -z "$ANDROID_NDK_HOME" ]; then
        echo "ОШИБКА: ANDROID_NDK_HOME не задан и не найден автоматически!"
        exit 1
    fi
fi
...
echo "=== talloc собран и установлен в $TALLOC_DIR ==="
...
# ARG_MAX=131072 — обход ограничения Android на размер аргументов execve
make -j$(nproc) \
    CC="$CC" \
    AR="$AR" \
    RANLIB="$RANLIB" \
...
# ============================================================
# ШАГ 3: Кладём бинарник в проект как .so
# ============================================================
echo ""
echo "=========================================="
...
echo "=== Готово! ==="
echo "Файл: $DEST_DIR/libproot.so"
